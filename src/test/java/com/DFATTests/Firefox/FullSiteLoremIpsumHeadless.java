...
public class FullSiteLoremIpsumHeadless {
	...
	public void A_DFAT_Crawler() throws InterruptedException, IOException
	{
		...
		for(int i=0;i<myUniqueURLString.size();i++)
		{
			...
			for( WebElement URL: myURLsList)
			{
				...
				try{
				if((!myUniqueURLString.contains(URL.getAttribute("href")))&&(URL.getAttribute("href")!= null)&&(URL.getAttribute("href").contains(myURL))&& (!URL.getAttribute("href").isEmpty())&&(!URL.getAttribute("href").endsWith("#"))&&(!URL.getAttribute("href").endsWith(".pdf")))
				{
			...
			count=0;
			if (!myUniqueURLString.get(i).contentEquals("https://www.asis.gov.au/Careers/Job-Vacancies.html")) {
